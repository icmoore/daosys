...
The Masternode Yield Farming tokenomics model has two main components: (a) Masternode rewards; and (b) DAOSYS rewards. From the a high-level perspective, users will commit the predetermined amount of liquidity to purchase a Masternode, the funds used for this purchase are staked thus used to generate $Coins_{New}$ in accordance to the specifications described in \cite{Sig21b}, which is as follows:
...
\end{itemize}
Using the rule set described in the above specifications, we determine the Masternode rewards using the following relationship: 
\begin{equation*}
MN_{Rewards} = (Blocks_{Num}) (Coins_{New}) \left(\frac{Nodes_{Num}}{Supply_{New}}\right)
\end{equation*}
where current supply is calculated via the following:
\begin{equation*}
Supply_{New} = Supply + Coins_{New} - Transaction Fee 
\end{equation*}
and where transaction fees were calculated in accordance to Section \ref{appendix:tx_fee_burn}. Using Masternode returns, we calculated the USD value using Monte Carlo price projections as per Section \ref{appendix:monte_carlo}. Final total returns were determined via the following: 
\begin{equation*}
...
To generate point predictions for the price of Syscoin, we made some assumptions. This is where price was given the freedom to fluctuate between \$0.25 to \$25 USD / SYS over the 5-year projection interval (ie, 2023-2028) while maintaining an overall upward logarithmic growth (like most successful crypto projects typically do). To do this we generated a set up Monte Carlo point predictions using the following:

\begin{equation}
\label{eqn:trend}
	\mathbf{y}_{t} = c + \mathbf{\tilde{z}}_{t}e^{k z_{t}}
\end{equation}
where the random drift term $\mathbf{\tilde{z}}_{t}$ normalized between the interval [$a_{lwr}$,$a_{upr}$] given by:
\begin{equation}
\label{eqn:min_max}
