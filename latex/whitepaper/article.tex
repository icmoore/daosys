...
\begin{equation*}
Supply_{New} = Supply_{Prev} + Coins_{New} - Fee_{TX} 
\end{equation*}
and $Fee_{TX}$ is calculated in accordance to Section \ref{appendix:tx_fee_burn}. Using Masternode returns, we calculated the USD value using Monte Carlo price projections as per Section \ref{appendix:monte_carlo}. Final total returns were determined via the following: 
\begin{equation*}
